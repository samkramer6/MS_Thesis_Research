...
# ╔═╡ 6073a3fe-e57c-11ee-16fa-5d5bc29d4fcf
begin
	using Plots
	using FFTW
	using StatsBase
...
# ╔═╡ 4a40cbd3-df95-4da9-b8a5-8ccc654d904f
md"## Basic Signal"

# ╔═╡ f390e1eb-85bb-4001-811b-2447eb2bf533
md"## Correlations"

# ╔═╡ 03c76be7-848e-4202-a377-9e25a3754ddb
md"## Function Definitions"

# ╔═╡ 4f7e8dc9-acc7-4331-af03-833c0f9b06c8
...
# ╔═╡ 4cc32419-13e8-456f-bac8-aa23dbe7dbb4
begin 
	...
	# --Create White noisy signal
		alpha = 10;
		rand_values = alpha .* vec(rand(Float64, 1, length(denoised_signal)));
	...
	# --Plotting out
		p1 = plot(time_long, white_noise_data, 
				label = false,
				title = "Noisy Signal",
				#ylim = (-120, 120),
				xlabel = "Time (s)",
		...
		annotate!(0.75, 1, text("SNR = $(noisy_data_SNR) dB", 8, "Computer Modern"))
...
# ╔═╡ 4054ec2f-111f-42cd-aa3e-36193ac77d06
...
# ╔═╡ 574c01a3-e8b2-4b76-bf54-c556de0d117e
...
# ╔═╡ 4c92273e-6b37-424b-b329-43de5a434e1b
function KCC(x::Vector{Float64}, y::Vector{Float64}, n::Int)
...
# ╔═╡ 6b2cdfe4-1387-45c9-a659-855bef6d81c0
...
# ╔═╡ 65937f16-b78b-4efa-b47f-a807aa24eeae
begin

	# --KCC with pure noise
		Noise_corr = KCC(white_noise, reference);
...
# ╔═╡ c593db99-7e16-4bd9-8976-1a6c78c36c86
...
# ╔═╡ 223b2d3d-7e46-43a4-9b73-f489b9737ab8
begin

	# --Normal Correlation
		corr = correlation(white_noise_data, reference);

	# --KCC L-Abs
		kcorr = KCC(white_noise_data, reference);

	# --KCC Polynomial 100
		kcorr100 = KCC(white_noise_data, reference, 100);

	# --Plotting
		p3 = plot(corr,
				tickfont = "Computer Modern",
				label = false,
				title = "Classic Correlation",
				titlefont = "Computer Modern")

		p4 = plot(kcorr100,
				tickfont = "Computer Modern",
				label = false,
				title = L"Polynomial Kernel Correlation $(n = 100)$",
				titlefont = "Computer Modern",
				ylim = (-.25, 1),
				xlabel = L"Lags (\tau)")
	
		annotate!(32000, 0.91, text("SNR = $(noisy_data_SNR) dB", 8, "Computer Modern"))
	
		plot(p3, p4, layout = [1, 1], dpi = 500)

end

# ╔═╡ 7cd47696-302e-47bf-bdbb-c454942930e8
begin

	# --ffts of the data
		fkcorr = abs.(fft(kcorr));
		fkcorr100 = abs.(fft(kcorr100));

	# --Put into dB scale
		fkcorr = 20 .* log10.(fkcorr);
		fkcorr100 = 20 .* log10.(fkcorr100);

	# --Find Frequencies
		frequencies = (0:1/5:fs);

	# --Plot
		plot(frequencies, fkcorr,
			xlim = (0, 750),
			xtick = 0:50:2000,
			label = false,
			tickfont = "Computer Modern",
			...
			ylabel = "Magnitude (dB)",
			guidefont = "Computer Modern",
			title = "Frequency Decomposition of L-Abs Kernel",
			titlefont = "Computer Modern")

	#savefig("Freq_Decom_of_Labs.pdf")

end

# ╔═╡ c33642b7-f36b-4447-9232-df9b57c19759
# --Plot
	plot(frequencies, fkcorr100,
			xlim = (0, 750),
			xtick = 0:50:2000,
			label = false,
			tickfont = "Computer Modern",
			...
			ylabel = "Magnitude (dB)",
			guidefont = "Computer Modern",
			title = "Frequency Decomposition of Polynomial Kernel",
			titlefont = "Computer Modern")
